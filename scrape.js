...
const scrape = async (city) => {
    ...
    await page.goto('https://www.wellsfargo.com/locator/bank/?slindex=1');
    ...
    const data = await page.evaluate(() => {
        ...
        branchInfo.address =
            ...
            ' ' +
            document.querySelectorAll('.adr .locality')[0].innerHTML.trim() +
            ', ' +
            document.querySelectorAll('.adr .region')[0].innerHTML.trim() +
            ', ' +
            document.querySelectorAll('.adr .postal-code')[0].innerHTML.trim();
        ...
        Array.from(document.querySelectorAll('.innerLeft ul')[1].children).map(
            (el) => {
